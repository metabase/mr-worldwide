...
(deftest ^:parallel localized-string?-test
  (is (i18n/localized-string? (i18n/deferred-trs "WOW")))
  (is (not (i18n/localized-string? "WOW"))))
...
(deftest ^:parallel validate-number-of-args-test-4
  (testing "`trs` and `tru` should validate that the are being called with the correct number of args\n"
    (testing "Missing format specifiers (e.g. {1} but no {0})"
...
(deftest ^:parallel validate-number-of-args-test-5
...
(deftest ^:parallel validate-number-of-args-test-6
